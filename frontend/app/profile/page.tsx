...
const ProfileSection: React.FC<{ data: VolunteerData }> = ({ data }) => (
  <div className="flex flex-col items-center p-6 border-r border-gray-300 w-80 text-center">
    ...
    <p className="text-gray-600">Volunteer Hours Completed: {data.volunteerWorkCount}</p>
    
    {/* Volunteer hours progress bar */}
    {data.volunteerWorkCount < 20000 && (
      <div className="w-full bg-gray-200 rounded-full h-4 mt-4">
        <div
          className="bg-blue-600 h-4 rounded-full"
          style={{ width: `${(data.volunteerWorkCount / getNextGoal(data.volunteerWorkCount)) * 100}%` }}
        ></div>
      </div>
    )}
    <p className="text-sm text-gray-600 mt-2 break-words">
      {data.volunteerWorkCount >= 20000
        ...
        : `Current Goal: ${getNextGoal(data.volunteerWorkCount)} Hours`}
    </p>

    {/* Milestone achievements */}
    <h3 className="mt-4 text-lg font-semibold">Milestone Achievements</h3>
    <ul className="list-disc ml-5 text-left">
      <li className={data.volunteerWorkCount >= 1000 ? "text-green-600" : "text-gray-400"}>1,000 Hours - Bronze Volunteer</li>
      <li className={data.volunteerWorkCount >= 5000 ? "text-green-600" : "text-gray-400"}>5,000 Hours - Silver Volunteer</li>
      <li className={data.volunteerWorkCount >= 10000 ? "text-green-600" : "text-gray-400"}>10,000 Hours - Gold Volunteer</li>
      <li className={data.volunteerWorkCount >= 20000 ? "text-green-600" : "text-gray-400"}>20,000 Hours - Platinum Volunteer</li>
    </ul>

    {/* Certification download buttons */}
    {data.volunteerWorkCount >= 1000 && (
  ...
  <div className="p-6">
    ...
    {data.events.length > 0 ? (
      <ul className="list-disc ml-5">
        {data.events.map((event: VolunteerEvent) => (
